...
func TestMain(m *testing.M) {
	...
	if testDatabaseURL == "" {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "app-b postgres setup skipped: %v\n", err)
		}
		testDatabaseURL = url
	}
	code := m.Run()
...
func startTestPostgres() (string, error) {
	name := "seleksilabpro-app-b-tests-" + uuid.NewString()
...
func stopTestPostgres() {
	...
	if out, err := exec.Command("docker", "rm", "-f", testContainerName).CombinedOutput(); err != nil {
		fmt.Fprintf(os.Stderr, "app-b postgres cleanup failed: %v: %s\n", err, strings.TrimSpace(string(out)))
	}
	testContainerName = ""
}
